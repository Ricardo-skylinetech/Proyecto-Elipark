                               ...
                               fn_tbl_bPases = function(partida_id){
                                ...
                                let tbl_recobros = $('#tbl_pases').DataTable({
                                    ...
                                    initComplete: function (settings, json) {
                                        if(json.comprobante != false){
                                            var ruta = json.comprobante[0].ruta;
                                            $("#comprobantePase").replaceWith(`<button class="dt-button verComprobante" onclick="window.open('../../detalle/viewFile/boletos_recobros/`+partida_id+`/`+ruta+`','name','width=1000,height=600')" target="_blank" data-toggle="tooltip" data-placement="top" title="Ver Comprobante"><span><i class="fas fa-eye fa-lg"></i></span></button>`);
                                        }
                                        fn_tbl_bValet(partida_id);
                                        $("#comprobantePase").on("click", function() {
                                            $("#tipoComprobante").val("boletos_pases");
                                            $("#comprobanteTipo").html("Comprobante Boletos Pases");
                                            $("#modalComprobantes").modal('show');
                                        });
                                    },
                                    footerCallback: function (row, data, start, end, display) {
