...
class Panel extends CI_Controller
{
    ...
    public function descargarLayout()
    {
        try {
            ...
            for ($i = 0; $i < count($cat_tarifa_pases); $i++) {
                $worksheet2->setCellValue("CM" . $fila_inicio, $cat_tarifa_pases[$i]["id"]);
                $worksheet2->setCellValue("CL" . $fila_inicio, $cat_tarifa_pases[$i]["tarifa"]);
                $worksheet2->setCellValue("CN" . $fila_inicio, $cat_tarifa_pases[$i]["estatus"]);
