...
class Pensiones extends CI_Controller
{
    ...
    public function getPensiones()
    {
        ...
        foreach ($data as $item) {
            ...
            elseif ($item['name'] == 'acumuladoIni') {
                $acumuladoIni = $item['value'];
            }
             elseif ($item['name'] == 'acumuladoFin') {
                $acumuladoFin = $item['value'];
            }
      ...
      $result = $this->mPensiones->getPensiones($select,$estacionamiento,$todo,$activas,$inactivas,$fechaMes,$filtroFoto,$acumuladoIni,$acumuladoFin);
        return respuesta_json($result);
    }
    ...
    public function sensarpensionesold(){
        ...
        if($result[0]['bandera']==1 && $ok[0]['bandera']==0){
            ...
            $this->db->update('cat_pensiones', array("estatus" => 0, "fechaAlta" =>'',"fechaBaja" =>date('Y-m-d')),  array("id_cat_pensiones"=>$id_cat_pensiones));
             ...
             $response->valida="Se han actualizado las pensiones";
              ...
              $response=new stdClass();
              $response->valida="2 todo bien, proxima fecha de inactivacion : ".$result[0]['proxInactivar'];
        }
