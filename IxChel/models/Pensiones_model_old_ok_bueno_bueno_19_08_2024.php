         ...
         $where = "`c`.`fechaAlta`=`p`.`fechaAlta`";
            ...
            // Traer pensiones del Corte 6 de cada mes
                ...
                $this->db->join('V_pensiones_gral p', "c.id_cat_pensiones = p.id_cat_pensiones");
                 $where = "`c`.`fechaAlta`=`p`.`fechaAlta` AND p.bandera=1";
                   $this->db->where($where);
...
//   return acc + (parseFloat(curr.venta) + parseFloat(curr.reposicion) + parseFloat(curr.recargos));
    ...
    public function guardarFicha($id_cat_pensiones, $nombre_archivo,$fechaUpdate) {
       ...
       /* $result = $this->db->update('pensiones_foto_mes', $data, $where);
        $result =  $this->db->update('pensiones_foto_mes', array("fichaPago" =>$nombre_archivo), array("id_cat_pensiones"=>$id_cat_pensiones));
        $result =  $this->db->update('pensiones_foto_mes', array("fechaAlta" =>$fechaUpdate), array("id_cat_pensiones"=>$id_cat_pensiones));


        $result =  $this->db->update('cat_pensiones_foto_mes', array("estatus" => 1), array("id_cat_pensiones"=>$id_cat_pensiones));
        $result =  $this->db->update('cat_pensiones_foto_mes', array("fechaAlta" =>$fechaUpdate), array("id_cat_pensiones"=>$id_cat_pensiones));*/
