...
class Pensiones_model extends CI_Model{
    ...
    public function getPensiones($select,$estacionamiento,$todo,$activas,$inactivas)
    {
        $year = date('Y');
        $month = date('m');
        ...
        $this->db->select("c.id_cat_pensiones,c.contrato,c.tarjetaSistema,c.tarjetaFisica,c.razonSocial,c.asignado,c.marca,c.modelo,c.color,c.placas,c.tipoPension,c.costo,c.factura,c.pago,c.venta,c.reposicion,c.recargos,c.fechaDeposito,c.movimiento,c.observaciones,p.fechaAlta, p.fechaBaja, p.estatus, p.fichaPago");
        ...
        if($activas == 1 && $inactivas == 1){
            $this->db->where_in(['p.estatus'=>array(1,0)]);
    ...
    {
        $this->db->select("c.id_cat_pensiones,c.contrato,c.tarjetaSistema,c.tarjetaFisica,c.razonSocial,c.asignado,c.marca,c.modelo,c.color,c.placas,c.tipoPension,c.costo,c.factura,c.pago,c.venta,c.reposicion,c.recargos,c.fechaDeposito,c.movimiento,c.observaciones,p.fechaAlta, p.fechaBaja, p.estatus, p.fichaPago, c.archivo, c.estacionamiento_id");
        $this->db->from("cat_pensiones c");
