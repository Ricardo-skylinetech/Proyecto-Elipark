...
class Pensiones_model extends CI_Model{
    ...
    public function getPensiones($select,$estacionamiento,$todo,$activas,$inactivas,$fechaMes,$filtroFoto,$acumuladoIni,$acumuladoFin)
    {
   ...
   // {
       ...
       // $this->db->join('pensiones p', "c.id_cat_pensiones = p.id_cat_pensiones AND p.fechaAlta BETWEEN '$finicio' AND '$ffin'","left");
        ...
        //$where = "`c`.`fechaAlta`=`p`.`fechaAlta` AND p.bandera=1 AND c.estatus=1";
         $where = "p.bandera=1 AND c.estatus=1";
            $this->db->where($where);
       ...
       // $this->db->join('pensiones p', "c.id_cat_pensiones = p.id_cat_pensiones AND p.fechaAlta BETWEEN '$finicio' AND '$ffin'","left");
        ...
        if(isset($acumuladoIni) && $acumuladoIni!="" && isset($acumuladoFin) && $acumuladoFin!=""){

            $acumuladoIni=$acumuladoIni."-01";
            $acumuladoFin=$acumuladoFin."-30";
            $where = "`p`.`fechaAlta` BETWEEN  '$acumuladoIni' AND '$acumuladoFin'";
            $this->db->where($where);
...
//   return acc + (parseFloat(curr.venta) + parseFloat(curr.reposicion) + parseFloat(curr.recargos));
                ...
                $fechaEntera = strtotime($fechaAlta);
    ...
    public function actualizarDatos($data) {
        ...
        //  print_r($fechaAlta[0]['fechaAlta']);
          ...
          /*if($fechaAlta[0]['fechaAlta']!=$fechaUpdate && $fechaUpdate!='00:00:00'){
